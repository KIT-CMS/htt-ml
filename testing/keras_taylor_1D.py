...
def main(args, config_test, config_train):
        ...
        for val_wei in uptree.iterate(expressions=variables+[weights],
            ...
            if args.no_abs:
                gradients = np.concatenate(
                    ([gradients_intermediate], gradients), axis=0)
            else:
                gradients = np.concatenate(
                    ([gradients_intermediate], np.abs(gradients)), axis=0)
            # Concatenate new weights to previous weights
            gradients_weights = np.concatenate(
                ([gradients_weights_intermediate], flat_weight), axis=0)
            # Get new itermediate averages and weights
            gradients_intermediate = np.average(gradients,
    ...
    plt.xticks(np.array(range(len(variables))) + 0.5,
               variables,
               rotation='vertical')
